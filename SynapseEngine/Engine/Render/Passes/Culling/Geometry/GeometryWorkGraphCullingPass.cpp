...
#include "Engine/Shaders/Includes/PushConstants/ModelMeshCullingPC.glsl"
    ...
    void GeometryWorkGraphCullingPass::Initialize() {
        ...
        config.defines.push_back(ShaderDefines::EnableWorkGraph);
        ...
        _shaderProgramId = shaderManager->LoadProgramAsync("GeometryWorkGraphCullingProgram", shaderFiles, config);
    }

    void GeometryWorkGraphCullingPass::CreateGraphPipeline() {
        auto device = ServiceLocator::Get<Vk::Context>()->GetDevice()->Handle();
        auto shaderManager = ServiceLocator::Get<ShaderManager>();
        ...
        std::vector<std::string> nodeNames = {
            "GeometryWorkGraphModelCullingNode",
            "GeometryWorkGraphStaticChunkCullingNode",
            "GeometryWorkGraphStaticModelCullingNode",
            "GeometryWorkGraphMortonChunkCullingNode",
        ...
        std::vector<std::string> defines = { ShaderDefines::EnableWorkGraph };

        for (uint32_t i = 0; i < 6; ++i) {
            auto shader = shaderManager->GetShader(shaderFiles[i], defines);
            SYN_ASSERT(shader != nullptr, "Shader not found for Work Graph!");
            SYN_ASSERT(shader->transientGpuData != nullptr, "Shader SPIR-V data is missing!");

            const auto& spirv = shader->transientGpuData->spirv;

            VkShaderModuleCreateInfo modInfo{ VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO };
            modInfo.codeSize = spirv.size() * sizeof(uint32_t);
    ...
    void GeometryWorkGraphCullingPass::PushConstants(const RenderContext& context) {
        ...
        uint32_t staticEntityCount = static_cast<uint32_t>(transformPool->GetStaticEntities().size());
        _staticChunkCount = drawData->Chunks.chunkCounter.load(std::memory_order_relaxed);
        _mortonChunkCount = ComputeGroupSize::CalculateDispatchCount(staticEntityCount, ComputeGroupSize::Buffer32D);
    ...
    void GeometryWorkGraphCullingPass::BindDescriptors(const RenderContext& context) {
        ...
        pushWriter.AddCombinedImageSampler(
            0,
            depthPyramid->GetView(),
            maxSampler->Handle(),
            VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL
        );

        pushWriter.Push(context.cmd, _shaderProgram->GetLayout(), 2, VK_PIPELINE_BIND_POINT_EXECUTION_GRAPH_AMDX);
    }
    ...
    void GeometryWorkGraphCullingPass::Dispatch(const RenderContext& context) {
        ...
        if (!dispatchInfos.empty()) {
            ...
            vkCmdDispatchGraphAMDX(
                context.cmd,
                _scratchBuffer ? _scratchBuffer->GetDeviceAddress() : 0,
                _scratchBuffer ? _scratchBuffer->GetSize() : 0,
                &countInfo
            );
        }
