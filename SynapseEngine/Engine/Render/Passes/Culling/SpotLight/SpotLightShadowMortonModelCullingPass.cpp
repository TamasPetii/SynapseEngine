#include "SpotLightShadowMortonModelCullingPass.h"
...
#include "Engine/Shaders/Includes/PushConstants/SpotLightShadowCullingPC.glsl"
    ...
    void SpotLightShadowMortonModelCullingPass::Initialize() {
        ...
        _shaderProgramId = shaderManager->LoadProgramAsync("SpotLightShadowMortonModelCullingProgram", {
            ShaderNames::SpotLightShadowMortonModelCullingComp
            }, config);
    }

    void SpotLightShadowMortonModelCullingPass::PushConstants(const RenderContext& context) {
    ...
    void SpotLightShadowMortonModelCullingPass::Dispatch(const RenderContext& context) {
        if (!_shouldDispatch) return;

        auto drawData = context.scene->GetSceneDrawData();
        uint32_t fIdx = context.frameIndex;

        VkBuffer cullBuffer = drawData->SpotLightShadow.mortonChunkDispatchBuffer.GetHandle(fIdx);
