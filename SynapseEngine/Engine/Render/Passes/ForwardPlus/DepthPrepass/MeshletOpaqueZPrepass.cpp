#include "MeshletOpaqueZPrepass.h"
...
namespace Syn {
    ...
    MeshletOpaqueZPrepass::MeshletOpaqueZPrepass(MaterialRenderType renderType)
        : _renderType(renderType)
    {
        ...
        if (_renderType == MaterialRenderType::Opaque1Sided) {
            _passName = "Meshlet_Opaque_ZPrepass_1Sided";
        }
        else {
            _passName = "Meshlet_Opaque_ZPrepass_2Sided";
        }
    }

    void MeshletOpaqueZPrepass::Initialize() {
        ...
        _shaderProgram = shaderManager->CreateProgram("MeshletOpaqueZPrepassProgram", {
    ...
    void MeshletOpaqueZPrepass::PrepareFrame(const RenderContext& context) {
    ...
    void MeshletOpaqueZPrepass::PushConstants(const RenderContext& context) {
    ...
    void MeshletOpaqueZPrepass::BindDescriptors(const RenderContext& context)
    {
        auto imageManager = ServiceLocator::GetImageManager();
        auto rtGroup = context.renderTargetManager->GetGroup(RenderTargetGroupNames::Deferred, context.frameIndex);
    ...
    void MeshletOpaqueZPrepass::Draw(const RenderContext& context)
    {
        auto scene = context.scene;
        auto drawData = scene->GetSceneDrawData();
        bool isGpu = scene->GetSettings()->enableGpuCulling;
