#include "TraditionalTransparentZPrepass.h"
...
#include "Engine/Shaders/Includes/PushConstants/TraditionalPassPC.glsl"
    ...
    TraditionalTransparentZPrepass::TraditionalTransparentZPrepass(MaterialRenderType renderType)
        : _renderType(renderType)
    {
        _passName = (_renderType == MaterialRenderType::Transparent1Sided) ? "Traditional_Transparent_ZPrepass_1Sided" : "Traditional_Transparent_ZPrepass_2Sided";
    }

    void TraditionalTransparentZPrepass::Initialize() {
        ...
        _shaderProgram = shaderManager->CreateProgram("TraditionalTransparentZPrepassProgram", {
            ShaderNames::TraditionalPreZVert,
            ShaderNames::TraditionalPreZFrag
            }, config);
    ...
    void TraditionalTransparentZPrepass::PrepareFrame(const RenderContext& context) {
    ...
    void TraditionalTransparentZPrepass::PushConstants(const RenderContext& context) {
    ...
    void TraditionalTransparentZPrepass::BindDescriptors(const RenderContext& context) {
        auto imageManager = ServiceLocator::GetImageManager();
    ...
    void TraditionalTransparentZPrepass::Draw(const RenderContext& context) {
        auto scene = context.scene;
        auto drawData = scene->GetSceneDrawData();
        bool isGpu = scene->GetSettings()->enableGpuCulling;
