#include "MeshletTransparentZPrepass.h"
...
namespace Syn {
    ...
    MeshletTransparentZPrepass::MeshletTransparentZPrepass(MaterialRenderType renderType)
        : _renderType(renderType)
    {
        _passName = (_renderType == MaterialRenderType::Transparent1Sided) ? "Meshlet_Transparent_ZPrepass_1Sided" : "Meshlet_Transparent_ZPrepass_2Sided";
    }

    void MeshletTransparentZPrepass::Initialize() {
        ...
        _shaderProgram = shaderManager->CreateProgram("MeshletTransparentZPrepassProgram", {
            ShaderNames::MeshletTask,
            ShaderNames::MeshletPreZMesh,
            ShaderNames::MeshletPreZFrag
            }, config);
    ...
    void MeshletTransparentZPrepass::PrepareFrame(const RenderContext& context) {
    ...
    void MeshletTransparentZPrepass::PushConstants(const RenderContext& context) {
    ...
    void MeshletTransparentZPrepass::BindDescriptors(const RenderContext& context)
    {
        auto imageManager = ServiceLocator::GetImageManager();
        auto rtGroup = context.renderTargetManager->GetGroup(RenderTargetGroupNames::Deferred, context.frameIndex);
    ...
    void MeshletTransparentZPrepass::Draw(const RenderContext& context)
    {
        auto scene = context.scene;
        auto drawData = scene->GetSceneDrawData();
        bool isGpu = scene->GetSettings()->enableGpuCulling;
