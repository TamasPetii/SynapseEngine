...
namespace Syn {
    ...
    void ClusterPointLightWritePass::PushConstants(const RenderContext& context) {
        ...
        pc.lightIndexListAddr = drawData->ForwardPlus.pointLightIndexBuffer.GetAddress(fIdx, true);

        pc.lightIndirectCmdAddr = drawData->PointLights.indirectBuffer.GetAddress(fIdx, isGpu);
