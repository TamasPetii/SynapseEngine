#include "MeshletTransparentGBufferPass.h"
...
namespace Syn {
    ...
    MeshletTransparentGBufferPass::MeshletTransparentGBufferPass(MaterialRenderType renderType)
        : _renderType(renderType)
    {
        _passName = (_renderType == MaterialRenderType::Transparent1Sided) ? "Meshlet_Transparent_1Sided" : "Meshlet_Transparent_2Sided";
    }

    void MeshletTransparentGBufferPass::Initialize() {
        ...
        _shaderProgram = shaderManager->CreateProgram("MeshletTransparentProgram", {
            ShaderNames::MeshletTask,
            ShaderNames::MeshletMesh,
            ShaderNames::MeshletTransparentFrag
            }, config);
    ...
    void MeshletTransparentGBufferPass::PrepareFrame(const RenderContext& context) {
    ...
    void MeshletTransparentGBufferPass::PushConstants(const RenderContext& context) {
    ...
    void MeshletTransparentGBufferPass::BindDescriptors(const RenderContext& context)
    {
        auto imageManager = ServiceLocator::GetImageManager();
    ...
    void MeshletTransparentGBufferPass::Draw(const RenderContext& context)
    {
        auto scene = context.scene;
        auto drawData = scene->GetSceneDrawData();
        bool isGpu = scene->GetSettings()->enableGpuCulling;
