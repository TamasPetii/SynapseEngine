#include "WireframeMeshletConePass.h"
...
#include "Engine/Shaders/Includes/PushConstants/WireframeMeshletPC.glsl"
    ...
    bool WireframeMeshletConePass::ShouldExecute(const RenderContext& context) const {
        return context.scene->GetSettings()->debug.enableWireframeMeshletCone;
    }

    void WireframeMeshletConePass::Initialize() {
        ...
        _shaderProgramId = shaderManager->LoadProgramAsync("WireframeMeshletConeProgram", {
    ...
    void WireframeMeshletConePass::PrepareFrame(const RenderContext& context) {
    ...
    void WireframeMeshletConePass::PushConstants(const RenderContext& context) {
        ...
        auto shape = modelManager->GetResource(MeshSourceNames::ProxyCone);
        ...
        pc->shapeType = WIREFRAME_MESHLET_SHAPE_TYPE_CONE;
        pc->materialRenderType = 0;
        pc->disableConeCulling = 1;
        pc.Push(context.cmd, _shaderProgram->GetLayout());
    }

    void WireframeMeshletConePass::BindDescriptors(const RenderContext& context) {
        auto imageManager = ServiceLocator::Get<ImageManager>();
    ...
    void WireframeMeshletConePass::Draw(const RenderContext& context) {
        auto scene = context.scene;
        auto drawData = scene->GetSceneDrawData();
        if (drawData->Models.activeMeshletCount == 0) return;
