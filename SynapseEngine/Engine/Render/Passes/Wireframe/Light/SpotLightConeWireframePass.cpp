...
#include "Engine/Shaders/Includes/PushConstants/WireframeDebugPC.glsl"
    ...
    {
        return context.scene->GetSettings()->debug.enableSpotLightConeWireframe;
    }
    ...
    void SpotLightConeWireframePass::PrepareFrame(const RenderContext& context) {
        ...
        copyRegion.dstBuffer = drawData->SpotLights.coneSingleCmdBuffer.GetHandle(fIdx);
    ...
    void SpotLightConeWireframePass::Draw(const RenderContext& context) {
        ...
        auto indirectBuffer = drawData->SpotLights.coneSingleCmdBuffer.GetHandle(fIdx);
