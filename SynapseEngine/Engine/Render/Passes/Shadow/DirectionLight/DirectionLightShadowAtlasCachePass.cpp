...
#include "DirectionLightShadowAtlasCachePass.h"
#include "Engine/Scene/Scene.h"
#include "Engine/Vk/Image/ImageUtils.h"
#include "Engine/Component/Light/Direction/DirectionLightShadowComponent.h"

namespace Syn {
    bool DirectionLightShadowAtlasCachePass::ShouldExecute(const RenderContext& context) const {
        auto pool = context.scene->GetRegistry()->GetPool<DirectionLightShadowComponent>();
        return pool && pool->Size() > 0;
    }

    void DirectionLightShadowAtlasCachePass::Execute(const RenderContext& context) {
        auto drawData = context.scene->GetSceneDrawData();
        uint32_t fIdx = context.frameIndex;

        auto srcDepth = drawData->DirectionLightShadow.shadowAtlas[fIdx].get();
        auto dstDepth = drawData->DirectionLightShadow.staticShadowAtlas[fIdx].get();
        auto srcColor = drawData->DirectionLightShadow.shadowColorAtlas[fIdx].get();
        auto dstColor = drawData->DirectionLightShadow.staticShadowColorAtlas[fIdx].get();
