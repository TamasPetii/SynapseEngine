#include "MeshletOpaqueAlphaTestedDeferredPass.h"
...
#include "Engine/Shaders/Includes/PushConstants/TraditionalMeshletPassPC.glsl"
    ...
    bool MeshletOpaqueAlphaTestedDeferredPass::ShouldExecute(const RenderContext& context) const
    {
        return context.scene->GetSettings()->lighting.pipelineType == PipelineType::Deferred;
    }

    MeshletOpaqueAlphaTestedDeferredPass::MeshletOpaqueAlphaTestedDeferredPass(MaterialRenderType renderType)
        : _renderType(renderType)
    {
        ...
        if (_renderType == MaterialRenderType::AlphaTestedOpaque1Sided) {
            _passName = "MeshletOpaqueAlphaTestedDeferred1Sided";
        }
        else {
            _passName = "MeshletOpaqueAlphaTestedDeferred2Sided";
        }
    }

    void MeshletOpaqueAlphaTestedDeferredPass::Initialize() {
        ...
        _shaderProgramId = shaderManager->LoadProgramAsync("MeshletOpaqueAlphaTestedDeferredProgram", {
    ...
    void MeshletOpaqueAlphaTestedDeferredPass::PrepareFrame(const RenderContext& context) {
    ...
    void MeshletOpaqueAlphaTestedDeferredPass::PushConstants(const RenderContext& context) {
    ...
    void MeshletOpaqueAlphaTestedDeferredPass::BindDescriptors(const RenderContext& context)
    {
        auto imageManager = ServiceLocator::Get<ImageManager>();
    ...
    void MeshletOpaqueAlphaTestedDeferredPass::Draw(const RenderContext& context)
    {
        auto scene = context.scene;
        auto drawData = scene->GetSceneDrawData();
