...
namespace Syn
{
    ...
    std::vector<TypeID> PointLightShadowRenderSystem::GetReadDependencies() const
    {
        return {
            TypeInfo<RenderSystem>::ID,
            TypeInfo<PointLightShadowSystem>::ID,
            TypeInfo<PointLightCullingSystem>::ID,
        };
    }

    std::vector<TypeID> PointLightShadowRenderSystem::GetWriteDependencies() const
    {
        return {
    ...
    void PointLightShadowRenderSystem::OnUpdate(Scene* scene, uint32_t frameIndex, float deltaTime, tf::Subflow& subflow)
    {
        ...
        this->EmplaceTask(subflow, SystemPhaseNames::Update, [this, scene, drawData]() {
                ...
                currentMainInstances == 0)
            {
                ...
                if constexpr (ENABLE_DEBUG_LOGGING) {
                    Info("[PointLightShadowRenderSystem] Capacity change detected. Main Instances: {}", currentMainInstances);
                }
            }
    ...
    void PointLightShadowRenderSystem::RebuildShadowBuffers(Scene* scene)
    {
        auto drawData = scene->GetSceneDrawData();
        auto& mainGroup = drawData->Models;
        auto& shadowGroup = drawData->PointLightShadow;

        uint32_t shadowTotalInstances = mainGroup.totalAllocatedInstances * POINT_SHADOW_MULTIPLIER;
        ...
        if constexpr (ENABLE_DEBUG_LOGGING) {
            Info("[PointLightShadowRenderSystem] Rebuilding Buffers. Shadow Instances: {}, Traditional Cmds: {}, Meshlet Cmds: {}",
    ...
    void PointLightShadowRenderSystem::OnUploadToGpu(Scene* scene, uint32_t frameIndex, tf::Subflow& subflow)
    {
        this->EmplaceTask(subflow, SystemPhaseNames::UploadGPU, [this, scene, frameIndex]() {
            ...
            if constexpr (ENABLE_DEBUG_LOGGING) {
                Info("[PointLightShadowRenderSystem] GPU Buffers Capacity Updated for Frame {}.", frameIndex);
            }
            });
    }

    void PointLightShadowRenderSystem::OnFinish(Scene* scene, tf::Subflow& subflow)
    {
