...
namespace Syn {
    ...
    std::vector<MaterialApiDesc> EditorApiImpl::GetAllMaterials() const {
        std::vector<MaterialApiDesc> result;

        auto materialManager = _engine->GetMaterialManager();
        if (!materialManager) return result;

        auto paths = materialManager->GetResourcePaths();

        for (const auto& path : paths) {
            uint32_t id = materialManager->GetResourceIndex(path);
    ...
    std::vector<TextureApiDesc> EditorApiImpl::GetAllTextures() const {
        ...
        for (const auto& path : paths) {
            uint32_t id = imageManager->GetResourceIndex(path);
            result.push_back({ id, path });
        }
    ...
    void EditorApiImpl::LinkTextureToMaterial(uint32_t materialId, uint32_t textureType, uint32_t textureId) {
        auto materialManager = _engine->GetMaterialManager();
        ...
        default:
            Syn::Warning("EditorApiImpl: Ismeretlen textúra slot ({}) a {} azonosítójú materialhoz!", textureType, materialId);
            return;
        }

        Syn::Info("EditorApiImpl: Textúra ({}) bekötve a Material ({}) {} slotjába.", textureId, materialId, textureType);

        //materialManager->UpdateMaterialGpu(materialId); 
    }

    void EditorApiImpl::UnlinkTextureFromMaterial(uint32_t materialId, uint32_t textureType) {
        auto materialManager = _engine->GetMaterialManager();
        if (!materialManager) return;
        ...
        Syn::Info("EditorApiImpl: Textúra kikötve a Material ({}) {} slotjából.", materialId, textureType);
